<?php
	class bishop extends piece {
		...
		public function displaypiece($color) {
			if($color=='white') {
				return "<img src=\"images/wb.gif\" />";
			} else if($color=='black') { 
				return "<img src=\"images/bb.gif\" />";
			}
		}
		...
		public function move($from, $to, $shortcode, $auth) {		
			if ($this->findcolor($shortcode)=="white") {
				if ($this->checkturn($auth)=="white") {
					if($this->whitebishopvalidate($from, $to, $shortcode, $auth)==True) {
				...
				if ($this->checkturn($auth)=="black") {
					if($this->blackbishopvalidate($from, $to, $shortcode, $auth)==True) {
						if ($this->incheck($from, $to, $shortcode, $auth)==false) {
		...
		public function whitebishopvalidate($from, $to, $shortcode, $auth) {
			$v = $from[1];
			$v2 = $from[1];
			$h = ord($from[0]);
			$h2 = ord($from[0]);
			...
			if ($from[1] < $to[1]) { 
				...
				while (($hmove == NULL) && ($h < '103') && ($h >= '97') && ($v2 < '8') && ($v2 >= '1')) {
					$move = chr(($h+1)).($v-1);
					$next = $h+1;
					$next02 = $v-1;
					$query = "SELECT $move FROM `chessboard` WHERE `auth`='$auth'";
		...
		public function blackbishopvalidate($from, $to, $shortcode, $auth) {
			$v = $from[1];
			$v2 = $from[1];
			$h = ord($from[0]);
			$h2 = ord($from[0]);
