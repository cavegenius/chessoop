<?php
	...
	class rook extends piece {
		...
		public function displaypiece($color) {
			if($color=='white') {
				return "<img src=\"images/wr.gif\" />";
			} else if($color=='black') { 
				return "<img src=\"images/br.gif\" />";
			}
		}
		...
		public function move($from, $to, $shortcode, $auth) {
			if ($this->findcolor($shortcode)=="white") {
				if ($this->checkturn($auth)=="white") {
					if($this->whiterookvalidate($from, $to, $shortcode, $auth)==True) {
				...
				if ($this->checkturn($auth)=="black") {
					if($this->blackrookvalidate($from, $to, $shortcode, $auth)==True) {
						if ($this->incheck($from, $to, $shortcode, $auth)==false) {
		...
		public function whiterookvalidate($from, $to, $shortcode, $auth) {
			$v = $from[1];
			$v2 = $from[1];
			$h = ord($from[0]);
			$h2 = ord($from[0]);
		...
		public function blackrookvalidate($from, $to, $shortcode, $auth) {
			$v = $from[1];
			$v2 = $from[1];
			$h = ord($from[0]);
			$h2 = ord($from[0]);
